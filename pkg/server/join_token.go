...
func (j *JoinToken) Sign(caPubKey []byte) {
	signer := hmac.New(sha256.New, j.SharedSecret)
	_, _ = signer.Write(caPubKey)
	j.Fingerprint = signer.Sum(nil)
}

func (j *JoinToken) VerifySignature(caPubKey []byte) bool {
	signer := hmac.New(sha256.New, j.SharedSecret)
	_, _ = signer.Write(caPubKey)
...
func (j *JoinToken) UnmarshalText(text []byte) error {
	decoder := base64.NewDecoder(base64.StdEncoding, bytes.NewReader(text))
...
func (j *JoinToken) Marshal() ([]byte, error) {
