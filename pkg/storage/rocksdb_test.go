...
func TestRocksDBGlobalSeqnumIssue(t *testing.T) {
	...
	key2 := MVCCKey{Key: []byte("ab"), Timestamp: hlc.Timestamp{WallTime:0x0100000000000000}}
	...
	db.Flush()
	db.Compact()
	// A snapshot ensures RocksDB assigns a nonzero global sequence number to the
	// SSTable we're about to ingest.
	...
	writer.Put(key1, valBase2)
	writer.Put(key2, valBase2)
	...
	// When this file is ingested, it'll be added to L0, since it overlaps with
	// the SSTable flushed earlier.
