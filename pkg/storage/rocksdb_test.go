...
// Regression test for https://github.com/facebook/rocksdb/pull/6843.
func TestRocksDBGlobalSeqnumIssueReverse(t *testing.T) {
	defer leaktest.AfterTest(t)()
	...
	keyBase := []byte("ab")
	keyBase2 := []byte{0x61, 0x62, 0x00}
	valBase := []byte("foobar")
	valBase2 := []byte("barfoo")
	key0 := MVCCKey{Key: []byte("aa")}

	// When encoded, this MVCC key is 0x616200000100000000000009, trailer
	// 0x0000000000000001.
	// Including the trailer, the internal key is encoded as
	// 0x6162000001000000000000090100000000000000.
	key1 := MVCCKey{Key: keyBase, Timestamp: hlc.Timestamp{WallTime: 0x0001000000000000}}
	// When encoded, this MVCC key is 0x61620000, trailer 0x0000000000000001 (seqnum
	// 0, key type 1 or SET), which gets encoded as little endian.
	// Including the trailer, this key is encoded internally as
	// 0x616200000100000000000000
	// Note that this internal key forms a prefix of the previous user key.
	key2 := MVCCKey{Key: keyBase2}
	...
	iter := db.NewIterator(IterOptions{LowerBound: key0.Key, UpperBound: roachpb.KeyMax})
	defer iter.Close()
	iter.SeekLT(MVCCKey{Key: []byte("ac")})
	if !iter.Key().Equal(key2) {
		t.Fatalf("expected keys to match: %v != %v", iter.Key(), key2)
	}
	val := iter.Value()
	iter.Prev()
	if !iter.Key().Equal(key1) {
		t.Fatalf("expected keys to match: %v != %v", iter.Key(), key1)
	}
	iter.Next()
	if !iter.Key().Equal(key2) {
		t.Fatalf("expected keys to match: %v != %v", iter.Key(), key1)
